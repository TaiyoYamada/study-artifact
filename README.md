...
Copyright (c) 2026 Taiyo Yamada

このリポジトリには性質の違うものが入っているので、分けて指定している。

| 対象                          | ライセンス                | 置き場所                         |
| ----------------------------- | ------------------------- | -------------------------------- |
| コード（サイト本体・ビルド）  | **GNU AGPL-3.0-or-later** | [`LICENSE`](LICENSE)             |
| ノート本文（`notes/` の文章） | **CC BY-NC-SA 4.0**       | [`notes/LICENSE`](notes/LICENSE) |

AGPL を選んでいるのは、これが Web サイトだから。GPL では「改変版を
自分でホスティングするだけならソース公開は不要」という抜け道ができるが、
AGPL はネットワーク越しに提供する場合も公開を求める（第 13 条）。
フォークして自分のノートサイトを立てるのは自由だが、その改変コードは
公開する必要がある。

ノート本文は非営利・継承の条件付き。引用や翻訳は歓迎するが、
広告付きのまとめサイトへ丸ごと転載されるのは想定していない。
なお、このライセンスが及ぶのは私が書いた文章だけで、各ノートが扱う
論文そのものの権利は各著者・出版社に帰属する。
